...
def read_credentials_from_file():
    # Read credentials from file and populate the dictionary
    try:
        ...
        # File not found, initialize an empty dictionary
        pass

def save_credentials_to_file():
    # Save credentials from dictionary to file
...
def login():
    ...
    login_window = Tk()
    login_window.geometry("1300x1200")
    ...
    username_entry = Entry(login_window, font=font)
    username_entry.pack(pady=5)

    password_label = Label(login_window, text="Password:", font=font)
    password_label.pack(pady=5)

    password_entry = Entry(login_window, font=font, show='*')
    ...
    def validate_credentials():
        ...
        if entered_username in credentials and credentials[entered_username] == hashed_password:
            # If the credentials are valid, close the login window and open the main GUI
    ...
    def open_signup_window():
        # Close the login window and open the signup window
        login_window.destroy()
        signup()

    # Create login button
    login_button = Button(login_window, text="Login", command=validate_credentials, font=button_font, bg="#4a90e2", fg='white', padx=10, pady=5)
    login_button.pack(pady=10)

    # Create signup button
    signup_button = Button(login_window, text="don't have account", command=open_signup_window, font=button_font, bg="#4a90e2", fg='white', padx=10, pady=5)
...
def signup():
    ...
    signup_window = Tk()
    signup_window.geometry("1300x1200")
    ...
    username_entry = Entry(signup_window, font=font)
    username_entry.pack(pady=5)

    password_label = Label(signup_window, text="Password:", font=font)
    password_label.pack(pady=5)

    password_entry = Entry(signup_window, font=font, show='*')
    password_entry.pack(pady=5)
    ...
    def create_account():
        ...
        if new_username in credentials:
            ...
            # Close the signup window and return to the login page
    ...
    login_button = Button(signup_window, text="have an account", command=open_login_window, font=button_font, bg="#4a90e2", fg='white', padx=10, pady=5)
    login_button.pack(pady=10)


    # Create signup button
    signup_button = Button(signup_window, text="Sign Up", command=create_account, font=button_font, bg="#4a90e2", fg='white', padx=10, pady=5)
...
def main_gui():
    ...
    main = tkinter.Tk()
    main.title("Text-Driven Facial Image Generation using Deep Convolutional Generative Adversarial Networks") #designing main screen
    ...
    def ganModel():
        ...
        if os.path.exists('model/generator_model_001.h5'):
            ...
            text.insert(END,'DC-GAN model generated\n')
            text.insert(END,'DC-GAN generated latent generated points size : '+str(X.shape)+"\n\n")
        else:
    ...
    def generateImage(data, features):
        global X, Y
        latent_points = generate_latent_points(200, 200) #making array of 200 to ask DC-GAN to generate 200 images from train model
    ...
    font = ('times', 16, 'bold')
    title = Label(main, text='A Realistic Image Generation of Face From Text Description Using the Deep Convolutional Generative Adversarial Networks')
    ...
    ganButton = Button(main, text="Generate DCGAN Model", command=ganModel)
